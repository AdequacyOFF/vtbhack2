...
class AuthService {
  ...
  bool get hasPendingConsents => banksWithPendingConsents.isNotEmpty;

  /// Refresh account consent status from bank
  Future<void> refreshAccountConsentStatus(String bankCode) async {
    try {
      // Check if we have a consent for this bank
      if (!_accountConsents.containsKey(bankCode)) {
        print('[AuthService] No consent found for $bankCode, cannot refresh');
        return;
      }

      final consentId = _accountConsents[bankCode]!.consentId;
      final service = getBankService(bankCode);

      // Get updated status from bank
      final updatedConsent = await service.getAccountConsentStatus(consentId, clientId);

      // Update stored consent
      _accountConsents[bankCode] = updatedConsent;
      await saveConsents();

      print('[AuthService] Refreshed consent status for $bankCode: ${updatedConsent.status}');
    } catch (e) {
      print('[AuthService] Error refreshing consent status for $bankCode: $e');
      // Don't throw, just log the error
    }
  }

  /// Refresh all consent types for a specific bank
  Future<void> refreshAllConsentsForBank(String bankCode) async {
    await refreshAccountConsentStatus(bankCode);

    // Also refresh payment and product consents if they exist
    try {
      if (_paymentConsents.containsKey(bankCode)) {
        final consentId = _paymentConsents[bankCode]!.consentId;
        final service = getBankService(bankCode);
        final updatedConsent = await service.getPaymentConsentStatus(consentId, clientId);
        _paymentConsents[bankCode] = updatedConsent;
      }
    } catch (e) {
      print('[AuthService] Error refreshing payment consent for $bankCode: $e');
    }

    try {
      if (_productConsents.containsKey(bankCode)) {
        final consentId = _productConsents[bankCode]!.consentId;
        final service = getBankService(bankCode);
        final updatedConsent = await service.getProductConsentStatus(consentId, clientId);
        _productConsents[bankCode] = updatedConsent;
      }
    } catch (e) {
      print('[AuthService] Error refreshing product consent for $bankCode: $e');
    }
  ...
  Future<Map<String, bool>> refreshAllConsents() async {
    ...
    for (final bankCode in supportedBanks) {
      try {
        ...
        print('[AuthService] Error refreshing consents for $bankCode: $e');
        results[bankCode] = false;
      }
    }
